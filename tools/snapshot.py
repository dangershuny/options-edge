...
def run_and_save(tickers: list[str] | None = None,
    ...
    for _, row in df_all[df_all["vol_signal"].isin(["BUY VOL", "FLOW BUY"])].iterrows():
        trade = {
            ...
            "news_drift_delta": row.get("news_drift_delta"),
            "news_event":       row.get("news_event"),
            "news_event_headline": row.get("news_event_headline"),
            "news_event_residual_pct": row.get("news_event_residual_pct"),
            "news_event_hours_elapsed": row.get("news_event_hours_elapsed"),
